// Dwelloro Property Intelligence Layer
// Deterministic risk scoring + seasonal patterns + cost intelligence + alerts.
// Pure functions over mongoose models passed in via context.
...
export function computeRiskBreakdown({ compliance = [], tickets = [], inspections = [] }) {
  ...
  // 2. Open ticket load weighted by urgency (0-20)
  const openTks = tickets.filter((t) => !['completed', 'closed'].includes(t.status));
  const ticketScore = Math.min(
    20,
    openTks.reduce((acc, t) => {
      const w =
        t.urgency === 'critical' ? 8 : t.urgency === 'high' ? 5 : t.urgency === 'medium' ? 2 : 1;
      return acc + w;
    }, 0)
  );

  // 3. Active Healthy Homes tickets (0-20)
  ...
  // 4. Major findings in latest inspection (0-15)
  ...
  const total = Math.min(
    100,
    complianceScore + ticketScore + hhScore + inspectionScore + staleScore
  );
    ...
    signals: [
      {
        ...
        max: 30,
        count: gaps.length,
        detail: gaps.map((g) => g.area),
        severity: sev(complianceScore, 6, 18),
      },
      {
        key: 'open_tickets',
        ...
        score: ticketScore,
        max: 20,
        count: openTks.length,
        severity: sev(ticketScore, 5, 12),
      },
      {
        key: 'healthy_homes',
        label: 'Active Healthy Homes issues',
        score: hhScore,
        max: 20,
        count: hhTks.length,
        severity: sev(hhScore, 5, 12),
      },
      {
        key: 'inspection_findings',
        label: 'Major inspection findings',
        score: inspectionScore,
        max: 15,
        count: majorFindings,
        severity: sev(inspectionScore, 3, 9),
      },
      {
        key: 'inspection_stale',
        label: 'Inspection recency',
        score: staleScore,
        max: 15,
        count: staleDays,
        severity: sev(staleScore, 5, 10),
      },
    ],
  };
}
...
export function seasonalPattern(tickets) {
  // Bucket last 4 quarters by category
  const cur = now();
  const quarters = [];
  for (let i = 3; i >= 0; i--) {
  ...
  // Trend: compare last 2 quarters to previous 2
...
// --- Trends (multi-month rollups) ---
const CATEGORY_COST_NZD_TRENDS = CATEGORY_COST_NZD; // alias to keep export clean
function monthKey(d) {
  const dt = new Date(d);
...
export function portfolioTrends({ properties, tickets, inspections }, months = 12) {
  ...
  // Category breakdown across full window
  const byCat = {};
  ...
  // Deteriorating properties: more tickets last 30d than prior 30d
  ...
  for (const p of properties) {
    ...
    if (delta >= 2 || (prior === 0 && recent >= 2)) {
      deteriorating.push({
        ...
        tickets_last_30d: recent,
        tickets_prior_30d: prior,
        delta,
      });
    }
  }
  ...
  // KPI deltas (last 3m vs prior 3m)
...
export function buildAlerts({ properties, compliance, tickets, inspections }) {
  ...
  // Compliance evidence stale (last_checked > 365d or never)
  ...
  // Overdue tickets: open > 30d OR critical/high not assigned within 48h
  for (const t of tickets) {
    if (['completed', 'closed'].includes(t.status)) continue;
    const days = daysSince(t.created_at);
    if (days > 30 && !['completed', 'closed'].includes(t.status)) {
      alerts.push({
        id: `tk-overdue-${t.id}`,
        severity: 'medium',
        type: 'ticket_overdue',
    ...
    if (
      ['critical', 'high'].includes(t.urgency) &&
      !t.assigned_contractor_id &&
      daysSince(t.created_at) > 2
    ) {
      alerts.push({
  ...
  // Inspection reminders: no inspection in 365d
  const inspByProp = {};
  for (const i of inspections) {
    const list = inspByProp[i.property_id] || [];
    list.push(i);
    inspByProp[i.property_id] = list;
  }
  for (const p of properties) {
    const list = (inspByProp[p.id] || []).sort(
      (a, b) => new Date(b.created_at) - new Date(a.created_at)
    );
    const latest = list[0];
    if (!latest) {
